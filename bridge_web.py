...
SERVICE_START_TIME = time.time()  # Bounty #90: uptime tracking
API_KEY_RATE_LIMITS = {
...
@app.route('/health')
def health():
    """Bounty #90: Extended health check with service monitoring."""
    # Uptime tracking
    ...
    services = {
        'database': 'ok' if (os.path.exists(DATA_FILE) and os.access(DATA_FILE, os.R_OK)) else 'degraded',
        'discord': 'ok' if os.environ.get('DISCORD_WEBHOOK_URL', '').strip() else 'degraded',
        'ai_api': 'ok' if (os.environ.get('GEMINI_KEY', '').strip() or os.environ.get('AI_API_KEY', '').strip() or os.environ.get('ANTHROPIC_KEY', '').strip()) else 'degraded'
    }
