    ...
    <div class="max-w-4xl mx-auto p-6">
        ...
        <div class="bg-gray-800 rounded-lg p-6">
            ...
            <div class="flex gap-4 flex-wrap">
                ...
                {% if pr.merged %}
                <form method="POST" action="{{ url_for('admin.reprocess_payout', pr_number=pr.number) }}"
                      onsubmit="return confirm('Re-process payout for this merged PR? This will queue a new payment.')">
                    <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 rounded font-medium transition">
                        🔄 Re-process Payout
                    </button>
                </form>
                {% endif %}
...
@admin_bp.route('/pr/<int:pr_number>/reprocess', methods=['POST'])
@login_required
def reprocess_payout(pr_number):
    ...
    from pr_security import extract_wallet_from_pr_body
    ...
    from api_webhooks import find_pr_review, queue_payment, get_bounty_amount, \
        notify_discord, truncate_wallet, post_github_comment, update_reputation
    
    review_data = find_pr_review(pr_number)
    review_score = None
    if review_data:
        review_score = review_data.get("review", {}).get("score")
    
    # Find bounty issue from PR body
    import re
    ...
    # Also try extracting from PR title/labels
    if not amount:
    ...
    queue_payment(pr_number, wallet, amount, bounty_issue_id=bounty_issue_id, 
                  review_score=review_score, author=pr_author)
    
    # Post GitHub comment
    try:
        post_github_comment(pr_number, 
            f"🔄 **Payment re-processed** — {amount:,} WATT queued for `{wallet[:8]}...{wallet[-8:]}`\n\n"
            f"_Re-processed by admin after wallet gate fix._")
    except:
        pass
    ...
    try:
        notify_discord(
            ...
            f"PR #{pr_number} | **{amount:,} WATT** queued for `{truncate_wallet(wallet)}`\n"
            f"_Admin re-process after wallet gate fix_",
            color=0x00AAFF,
            fields={"PR": f"#{pr_number}", "Author": f"@{pr_author}", 
                     "Amount": f"{amount:,} WATT", 
                     "Issue": f"#{bounty_issue_id}" if bounty_issue_id else "N/A"}
        )
    except:
        pass
